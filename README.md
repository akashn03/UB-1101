...
When deploying the Next.js frontend to Vercel, be sure to set the **Root Directory** to `frontend` so Vercel builds the correct part of the repository. Populate the Environment Variables in the Vercel dashboard matching your `.env.local` file, replacing `NEXT_PUBLIC_SITE_URL` with your actual Vercel domain.

### Deploying the Backend (Render / Railway)
When deploying the FastAPI backend to a service like Render:
1. Set the **Root Directory** to `backend` (Crucial so it finds the `requirements.txt` file).
2. Start Command: `uvicorn main:app --host 0.0.0.0 --port $PORT`
3. Add your `GEMINI_API_KEY` to the environment variables.
4. Once deployed, take the public URL (e.g., `https://healmycity-api.onrender.com`) and update your frontend's `NEXT_PUBLIC_API_URL` variable in Vercel to reflect the live production endpoint.
